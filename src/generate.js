...
const username = () => {
  ...
  while (prefix.length + name.length + suffix.length < config.username.length) {
    ...
    if (
      prefix.length + name.length + smallestCharLength + suffix.length >
      config.username.length
    ) {
      return username()
    }
    let character =
      config.username.characters[
        Math.floor(Math.random() * config.username.characters.length)
      ]
    if (
      prefix.length + name.length + character.length + suffix.length <=
      config.username.length
    ) {
      name += character
    }
